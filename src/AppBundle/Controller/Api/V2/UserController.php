...
public $db;
            ...
            foreach ($users as $user) {
                if (is_object($user)) {
                    ...
                    $newUsers[] = array(
                        ...
                        'photo' => $mainPhoto,
                        'url' => $fotoMain,
                        //'username' => mb_strlen($user->getUsername(),'UTF-8') < 12 ? $user->getUsername() : substr($user->getUsername(),0,12),
    ...
    public function postSettingsAction(Request $request){
        ...
        if(isset($post['is_sent_email']) or isset($post['is_sent_push'])){
            ...
            $em->persist($user);
            $em->flush();
            $success = $this->get('translator')->trans('ההגדרות נשמרו בהצלחה');
    ...
    public function postLoginAction(){
        ...
        $isPay = ($settings->getIsCharge() or $user->getId() == 111 or $user->getId() == 124 or $user->getId() == 3348) ? $user->isPaying() : true;
    ...
    public function getUserAction(User $user, Request $request)
    {
        ...
        //if(!isset($_REQUEST['test'])){
           // return $this->getAllUserPhotos($user);
        //}//$this->getPhotosAction($request, false);
        $purposes = '';
        ...
        foreach ($user->getAnimals() as $animal){
            $animals .= ((empty($animals)) ? '' : ', ') . $animal->getName();
        }

        $religionAffinity = $user -> getReligionAffinity();
        $religionAffinity = $religionAffinity != null ? $religionAffinity->getName() : false;
        ...
        $res = array(
            ...
            'religionAffinity' => array('label' => $this->get('translator')->trans('זיקה לדת'), 'value' => $religionAffinity),

            'relationshipStatus' => array('label' => $this->get('translator')->trans('מצב משפחתי'), 'value' => $user->getRelationshipStatus()->getName()),
            'iam' => array('label' => $this->get('translator')->trans('אני'), 'value' => $user->getRelationshipStatus()->getName()),
            'type' => array('label' => $this->get('translator')->trans('אני'), 'value' => $user->getType()->getName()),

            'zodiac' => array('label' => $this->get('translator')->trans('מזל'), 'value' => $user->getZodiac()),
            'music' => array('label' => $this->get('translator')->trans('המוזיקה שאני שומע'), 'value' => $user->getMusic()),
            ...
            'animals' => array('label' => $this->get('translator')->trans('בע״ח'), 'value' => $animals),
            ...
            'dinner' => array('label' => $this->get('translator')->trans('איזה אישיות הייתי מזמין לארוחת ערב?'), 'value' => $user->getDinnerWith()),
            'veggieReasons' => array('label' => $this->get('translator')->trans('אני צמחוני/טבעוני כי'), 'value' => $veggieReasons),
            'nutrition' => array('label' => $this->get('translator')->trans('תזונה'), 'value' => $user->getNutrition()?$user->getNutrition()->getName():''),
...
//        var_dump(count($userPhotos), count($userPhotos[0]));
//        if(count($userPhotos) == 2 && count($userPhotos[0]) == 2) {
//            $userPhotos[0]['isMain'] == true;
//        }
    ...
    public function postPhotoAction(Request $request)
    {
        ...
        if($uploadedPhoto instanceof UploadedFile) {
            ...
            $isMain = ($user->getMainPhoto(true)) ? false : true;
            $isValid = false;
    ...
    */
    ...
    public function postLikeAction(User $toUser)
    {
        ...
        $result = $user->sendUserLike($this->get('security.token_storage')->getToken()->getUser(), $toUser);
    ...
    public function getBingoAction(Request $request)
    {
     ...
     //    var_dump($request->get('likeMeId'));
        ...
        $res['texts'] = array(
            'text' => $this->get('translator')->trans(' גם USERNAME בעניין שלך!'),
            'send' => $this->get('translator')->trans('דברו איתו/ה'),
            'cancel' => $this->get('translator')->trans('המשיכו לדפדף'),
            'photo' => (is_object($user->getMainPhoto())) ? $user->getMainPhoto()->getFaceWebPath() : $user->getNoPhoto(),
...
//                    }
    ...
    public function postListsAction(Request $request, User $member)
    {
        ...
        $message =  $member->getUsername() . ' ' . (($action == 'create') ? 'התווסף בהצלחה '.$txt : ' הוסר בהצלחה '.$txt);
    ...
    public function getFreezeAction()
    {
        ...
        return $this->view( array(
            ...
            'description' => $this->get('translator')->trans('לידיעתכם: הקפאת פרופיל גורמת לכך שכרטיסך לא יופיע באתר בתקופת ההקפאה, אך עדין יהיה לך כרטיס קיים שתוכל לחזור ולהשתמש בכל עת. כדי לבטל את ההקפאה יש לבצע כניסה לאתר עם שם משתמש וסיסמה. לאחר תקופה של הקפאה המערכת מסירה אוטומטית את החשבונות הלא פעילים שבאתר. הקפאת חשבון אינה מבטלת את המנוי בתשלום במידה ויש לך כזה. כדי לבטל את המנוי בתשלום טרם ההקפאה יש לשלוח לנו מייל דרך עמוד צור קשר, ובו בקשה לביטול המנוי בתשלום. מנוי אשר נרכש דרך iTunes יש לבטל באופן עצמאי דרך חשבונכם באייטיונז.'),
    ...
    public function postReportAbuseAction(Request $request, User $member)
    {
        ...
        // $subject = "GreenDate | Report Abuse | Username: " . $member->getUsername();
    ...
    public function getEditProfileAction(Request $request){
        ...
        $user = $this->get('security.token_storage')->getToken()->getUser();
      //var_dump($user->getPurposes());die;
       // var_dump($user>getEmail());die;
        switch ($step){
            case '1':
                $formType = new ProfileOneApiType();
                break;
            case '2':
       ...
       // var_dump($this->transformForm($form));die;
        return $this->view(array(
            'form' => $this->transformForm($form),
    ...
    public function postEditProfileAction(Request $request){
        ...
        //return $this->view($post['about'], Response::HTTP_OK);
        //var_dump($form->has('email'));die;
        if($form->has('email')) {
          // var_dump( $form->get('email'));
           ...
           // $errors = true;
//            if(!$emailInBlocked){
//                $emailInBlocked = $this->getDoctrine()->getManager()->getRepository('AppBundle:User')->findOneByEmail(strtolower($form->get('email')->get('first')->getData()));
//                if($emailInBlocked and $emailInBlocked->getId() == $user->getId()){
//                    $emailInBlocked = null;
//                }
//            }
            if($emailInBlocked) {
        ...
        if($form->has('about') and empty($post['about'])){
            $form->get('about')->addError(new FormError($this->get('translator')->trans('שדה ״קצת עליי״ חייב להכיל לפחות 10 תוים')));
            $errors = true;
        }
        //looking
        if($form->has('looking') and empty($post['looking'])){
            $form->get('looking')->addError(new FormError($this->get('translator')->trans('שדה ״מה אני מחפש/ת״ חייב להכיל לפחות 10 תוים')));
            $errors = true;
        }
        //hobbies
        ...
        if($form->has('username') and empty($post['username'])){
            $form->get('username')->addError(new FormError($this->get('translator')->trans('נא להזין שם משתמש')));
            $errors = true;
        }
//        if($form->has('religionAffinity') and empty($post['religionAffinity'])){
//            $form->get('religionAffinity')->addError(new FormError($this->get('translator')->trans('נא לבחור זיקה לדת')));
//            $errors = true;
//        }
//        if($form->has('purposes') and count((array)($post['purposes'])) < 1){
//            $form->get('purposes')->addError(new FormError($this->get('translator')->trans('נא לבחור לפחות מטרה אחת')));
//            $errors = true;
//        }
//        if($form->has('phone') and !empty($post['username']) and !is_numeric($post['phone'])){
//            $form->get('phone')->addError(new FormError($this->get('translator')->trans('טלפון יכול להכיל רק מספרים')));
//            $errors = true;
//        }
...
//            $textAfter->setText('
//			<ul>
//				<li>התשלום יגבה באמצעות חשבון ה-ITunes לאחר אישור הרכישה.</li>
//			    <li>החיוב מתבצע 24 שעות לאחר מועד הרכישה.</li>
//			    <li>המנוי מתחדש באופן אוטומטי אלא אם המשתמש מודיע על ביטול לפחות 48 שעות לפני מועד החידוש.</li>
//			    <li>חידוש המנוי יחוייב 24 שעות לפני סיום התקופה המקורית, לפי תנאי הרכישה שאושרו על ידי המשתמש.</li>
//			    <li>ניתן לנהל את הגדרות המנוי והגדרות החידוש האוטמטי דרך הגדרות חשבון ה-ITunes.</li>
//
//
//			</ul>');
...
//            $content = '';
//            foreach ($textBefore as $textBeforeText){
//                $content .= $textBeforeText->getText();
//            }
//            $textBeforeN = new TextBeforePayment();
//            $textBeforeN->setText($content);
//            $textBefore = array($textBeforeN);
        }
        //payPeriod => payment.getPeriod()
        //prc => btoa('{{ payment.amount }}')
        return $this->view(array(
            'url' => 'https://redirect.telepay.co.il?formId=5265d2d0-cdc7-e911-b80c-ecebb8951f7e&userId=' . $userId,
       ...
       // $userId = $this->getUser()->getId();
        ...
        if ($user->getId()) {
            if ($monthsNumber) {
                    ...
                    // + add save in db
                    return $result;
//                 $users->output(array("result" => $result, 'userId' => $userId,'num' => $monthsNumber));
                ...
                $headers = "MIME-Version: 1.0" . "\r\n";
                $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
                $headers .= 'From: do_not_reply@greendate.co.il <do_not_reply@greendate.co.il>' . "\r\n";
                //mail('albert@interdate-ltd.co.il','Shedate Restore',serialize($body) . $monthsNumber,$headers);
    //            echo($history);
             ...
             //   var_dump($history);
               ...
               // var_dump($history);
                ...
                switch ($subscribe['productId']) {
                    case 'greendate.oneWeek':
                        $monthsNumber = 7;
                        break;
                    case 'greendate.oneMonth':
                        $monthsNumber = 1;
                        break;
                    case 'greendate.threeMonths':
                        $monthsNumber = 3;
                        break;
                    case 'greendate.sixMonth':
                        $monthsNumber = 6;
                        break;
                    case 'greendate.oneYear':
                        $monthsNumber = 12;
                        break;
                }
...
//                        'dateNow' => $dateNow,
//                        'subendat' => $sub_end_at,
//                        'isPaing' => $user->isPaying(),
//                        'dateLast' => $dateLast,
//                        'strtoDateLat' => strtotime($dateLast),
//                    );
