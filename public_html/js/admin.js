    ...
    function () {
        ...
        $('.users .delete').click(function(){
            ...
            if($('.sel_item .ui.checkbox input[type="checkbox"]').is(":checked")){
                if( confirm('להסיר משתמשים מסומנים?') ){
                    $('.sel_item .ui.checkbox').each(function(){
                        if($(this).find('input[type="checkbox"]').is(":checked")){
...
function getSubscr(id){
    ...
    $.ajax({
        url: '/app_dev.php/admin/users/user/' + id + '/subscription',
        type: 'Get',
        error: function(response){
            console.log("Error:" + JSON.stringify(response));
        },
...
function setDatePeriods(objRef_l, objRef_h, period, dtformat) {
    ...
    switch (period) {
        ...
        case 6:
        case 12:
            var tempArr = objRef_l.val().split('-');
            if(tempArr[0].length >= 4) {
                ...
                d = new Date(tempArr[2] + '-' + tempArr[1] + '-' + tempArr[0]);
                //alert(d.getMonth());
            }
            dStr_l = createdStr();
