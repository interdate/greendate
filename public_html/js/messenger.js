	...
	this.processMessage = function(message){ 
		...
		message.text = (!message.text.length && !this.allowedToReadMessage)
			? (Messenger.currentUserHasPoints) 
				? $('#paymentText').val() + ' <a href="' + $('#paymentLink').val() + '">' + $('#paymentLinkText').val() + '</a> או <a onclick="Messenger.useFreePointToReadMessage(this)" class="usePoint">' + $('#pointsLinkText').val() + '</a>'
	...
	*/
	...
	this.sendMessage = function(textarea){		
		...
		*/
		...
		$.ajax({
			url: '/app_dev.php/messenger/message/send/userId:' + Messenger.currentUserId + '/contactId:' + this.contactId,
			headers: { 'apiKey': Messenger.apiKey },
	...
	this.sendPush = function(){
		$.ajax({
			url: '/messenger/message/push/userId:' + Messenger.currentUserId + '/contactId:' + this.contactId,
			headers: {'apiKey': Messenger.apiKey},
			timeout: 80000,
			dataType: 'json',
			type: 'Post',
