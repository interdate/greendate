...
class DriverChainTest extends DoctrineTestCase
{
    public function testDelegateToMatchingNamespaceDriver()
    {
        ...
        $chain->addDriver($driver1, 'DQL\Tests\Models\Company');
        $chain->addDriver($driver2, 'DQL\Tests\Common\Persistence\Mapping');
    ...
    {
        $className = 'Doctrine\Tests\Common\Persistence\Mapping\DriverChainEntity';
        $classMetadata = $this->getMock('DQL\Common\Persistence\ClassMetadata');
        ...
        $driver1->expects($this->once())
                ...
                ->will($this->returnValue(array('DQL\Tests\Models\Company\Foo')));

        $driver2 = $this->getMock('Doctrine\Common\Persistence\Mapping\Driver\MappingDriver');
        $driver2->expects($this->once())
                ->method('getAllClassNames')
                ->will($this->returnValue(array('DQL\Tests\ORM\Mapping\Bar', 'DQL\Tests\ORM\Mapping\Baz', 'FooBarBaz')));

        $chain->addDriver($driver1, 'DQL\Tests\Models\Company');
        $chain->addDriver($driver2, 'DQL\Tests\ORM\Mapping');

        $this->assertEquals(array(
            'DQL\Tests\Models\Company\Foo',
            'DQL\Tests\ORM\Mapping\Bar',
            'DQL\Tests\ORM\Mapping\Baz'
    ...
    public function testIsTransient()
    {
        $driver1 = $this->getMock('Doctrine\Common\Persistence\Mapping\Driver\MappingDriver');
        $chain = new MappingDriverChain();
        $chain->addDriver($driver1, 'DQL\Tests\Models\CMS');
    ...
    public function testDefaultDriver()
    {
        ...
        $entityClassName    = 'DQL\Tests\ORM\Mapping\DriverChainEntity';
        $managerClassName   = 'DQL\Tests\Models\Company\CompanyManager';
        $chain              = new MappingDriverChain();
        ...
        $chain->addDriver($companyDriver, 'DQL\Tests\Models\Company');
    ...
    public function testDefaultDriverGetAllClassNames()
    {
        ...
        $this->assertEquals(array('DQL\Tests\Models\Company\Foo', 'Other\Class'), $classNames);
    }
}
