        ...
        foreach ($this->registry->getManagers() as $em) {
            ...
            if (!is_dir($proxyCacheDir = $em->getConfiguration()->getProxyDir())) {
                if (false === @mkdir($proxyCacheDir, 0777, true)) {
                    throw new \RuntimeException(sprintf('Unable to create the DQL Proxy directory "%s".', $proxyCacheDir));
                }
            } elseif (!is_writable($proxyCacheDir)) {
                throw new \RuntimeException(sprintf('The DQL Proxy directory "%s" is not writeable for the current system user.', $proxyCacheDir));
            }
