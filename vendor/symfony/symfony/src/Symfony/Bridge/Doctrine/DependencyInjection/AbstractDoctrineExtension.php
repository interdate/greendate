...
/**
 * This abstract classes groups common code that DQL Object Manager extensions (ORM, MongoDB, CouchDB) need.
 *
...
abstract class AbstractDoctrineExtension extends Extension
{
     ...
     *
     * Aliases can be used in the Query languages of all the DQL object managers to simplify writing tasks.
     *
    ...
    protected function setMappingDriverConfig(array $mappingConfig, $mappingName)
    {
        ...
        if (!is_dir($mappingDirectory)) {
            throw new \InvalidArgumentException(sprintf('Invalid DQL mapping path given. Cannot load DQL mapping/bundle named "%s".', $mappingName));
        }
    ...
    protected function assertValidMappingConfiguration(array $mappingConfig, $objectManagerName)
    {
        if (!$mappingConfig['type'] || !$mappingConfig['dir'] || !$mappingConfig['prefix']) {
            throw new \InvalidArgumentException(sprintf('Mapping definitions for DQL manager "%s" require at least the "type", "dir" and "prefix" options.', $objectManagerName));
        }

        if (!is_dir($mappingConfig['dir'])) {
            throw new \InvalidArgumentException(sprintf('Specified non-existing directory "%s" as DQL mapping source.', $mappingConfig['dir']));
        }
    ...
    protected function loadCacheDriver($cacheName, $objectManagerName, array $cacheDriver, ContainerBuilder $container)
    {
        ...
        switch ($cacheDriver['type']) {
            ...
            case 'zenddata':
                ...
                break;
            default:
                throw new \InvalidArgumentException(sprintf('"%s" is an unrecognized DQL cache driver.', $cacheDriver['type']));
        }
