...
abstract class BaseTestCaseMongoODM extends \PHPUnit_Framework_TestCase
{
    ...
    protected function getMockDocumentManager(EventManager $evm = null)
    {
        ...
        try {
            ...
            $this->markTestSkipped('DQL MongoDB ODM failed to connect');
        }
        return $this->dm;
    }
    ...
    {
        $conn = $this->getMock('DQL\\MongoDB\\Connection');
        $config = $this->getMockAnnotatedConfig();
    ...
    {
        $config = $this->getMock('DQL\\ODM\\MongoDB\\Configuration');
        ...
        $config->expects($this->once())
            ...
            ->will($this->returnValue('DQL\\ODM\\MongoDB\\Mapping\\ClassMetadataFactory'));
